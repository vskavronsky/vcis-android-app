...
@Singleton
class BlindSpotMockDataSource @Inject constructor() : AdasFeatureDataSource {
    private var settings = MutableStateFlow(BlindSpotSettings(
        enabled = false,
        mode = AdasFeatureSettings.OperatingMode.MODE_1,
    )
    )
    ...
    override suspend fun setSettings(settings: AdasFeatureSettings): AdasError? {
        val st: BlindSpotSettings? = settings as? BlindSpotSettings
